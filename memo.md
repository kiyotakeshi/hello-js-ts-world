...
---
- メモ化
    - 関数内における任意のサブルーチーンを呼び出した結果を後で再利用するために保持しておき、関数が呼び出されるたびに再計算されることを防ぐ、プログラミングの高速化の手法
    - 計算結果をメモしておく
    - 関数コンポーネントの中に計算リソースを多大に消費する処理が内包され、レンダリングごとに実行されるのは無駄

---
- Custom Hook でコンポーネントから Hooks のロジックを切り出す
    - Custom Hook 関数の名前の頭は use から始める
